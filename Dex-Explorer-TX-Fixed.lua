...
env.decompile = function(scr)
    local function b64_encode(data)
        ...
        for i = 1, #data, 3 do
            ...
            local b1, b2, b3 = bytes[1], bytes[2] or 0, bytes[3] or 0
            ...
            if #bytes >= 2 then table.insert(result, b64chars:sub(c3 + 1, c3 + 1)) else padding = padding + 1 end
            if #bytes >= 3 then table.insert(result, b64chars:sub(c4 + 1, c4 + 1)) else padding = padding + 1 end
        end
        for i = 1, padding do table.insert(result, '=') end
        return table.concat(result)
    end
    local ok, bytecode = pcall(getscriptbytecode, scr)
    if not ok or type(bytecode) ~= "string" then
        return ("-- DEX - Failed to read bytecode of %s\n--[[\n%s\n--]]"):format(scr.ClassName, tostring(bytecode))
    end
    local encoded = b64_encode(bytecode)
    local httpService = game:GetService("HttpService")
    local body = httpService:JSONEncode({ script = encoded })
    local requestFunc = env.request or (syn and syn.request) or http_request or (http and http.request) or fluxus and fluxus.request
    if not requestFunc then
        return "-- DEX - No HTTP request function available for lua.expert API"
    end
    ...
    if not success or not response then
        return ("-- DEX - HTTP request failed: %s\n--[[\n%s\n--]]"):format(tostring(err), tostring(response))
    end
    if response.StatusCode ~= 200 then
        return ("-- DEX - lua.expert API error (HTTP %d)\n--[[\n%s\n--]]"):format(response.StatusCode, tostring(response.Body))
    end
    return "-- decompile Fixed by TX \n" .. response.Body or ""
end
	...
	Main.CreateIntro = function(initStatus) -- TODO: Must theme and show errors
		local gui = create({
			...
			{11,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Creator",Parent={2},Position=UDim2.new(1,-110,1,-20),Size=UDim2.new(0,105,0,20),Text="Developed by Moon & TX反编译库修复",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=1,}},
...
-- Start
Main.Init()-- https://github.com/LorekeeperZinnia/Dex
