...
--[[
    ...
    Decompile optimize Carrying lua expert
...
if AnticheatData.Disabled then
	wax.shared.Sonner.success(`Cobalt 已绕过反作弊 {AnticheatData.Name} (反作弊删除)`)
end
		...
		Instance[Method] = function(...)
			...
			if
				LogRemote(Instance, Method, Callback, {
					...
					Function = Callback,
					Line = nil,
					IsExecutor = Callback and isexecutorclosure(Callback) or false,
				}, ...)
			then
				return
			end
		...
		if self.ClassName == "RemoteFunction" or self.ClassName == "BindableFunction" then
			...
			if key == Method and IsCallable then
				return wax.shared.NewIndexHook(self, key, function(...)
					...
					if
						LogRemote(self :: InstancesToHook, Method, value, {
							Arguments = table.pack(...),
							Time = os.date("%X"),
							Origin = Origin,
							Function = value,
							Line = nil,
							IsExecutor = value and isexecutorclosure(value) or false,
						}, ...)
					then
						return
					end
...
function CodeGen.GetFullPath(Object, ExcludeCode, VariableName, IsInTable)
	if
		rawequal(Object, game)
		or (wax.shared.ExecutorSupport["compareinstances"].IsWorking and compareinstances(Object, game))
	then
		return "game"
	end
	...
	if typeof(ChildLookupMode) == "boolean" then
		ChildLookupMode = if ChildLookupMode then "WaitForChild" else "Index"
		wax.shared.SaveManager:SetState("InstancePathLookupChain", ChildLookupMode)
	end
	if ChildLookupMode ~= "FindFirstChild" and ChildLookupMode ~= "WaitForChild" and ChildLookupMode ~= "Index" then
...
%s]],
			...
			CallInfo.Function.IsC and "<b>闭包类型</b>: C closure"
				or string.format(
					"<b>闭包类型</b>: Luau closure\n<b>常亮:</b> %s\n<b>上值:</b> %s\n<b>原型:</b> %s",
...
%s]],
		...
		iscclosure(CallInfo.Function) and "<b>闭包类型</b>: C closure"
			or string.format(
				"<b>闭包类型</b>: Luau closure\n<b>常亮:</b> %s\n<b>上值:</b> %s\n<b>原型:</b> %s",
...
local function CreateSettingsTextBox(
	...
	local DefaultValue = if typeof(Options.Default) == "string" then Options.Default else ""
	local SavedValue = wax.shared.SaveManager:GetState(Idx)
	local InitialValue = if typeof(SavedValue) == "string" then SavedValue else DefaultValue
	...
	function TextSetting:SetValue(NewValue)
		...
		if NewValue == TextSetting.Value then
			SyncValue(NewValue, false)
			return
		end
...
local function CreateSettingsRemoteList(
	...
	RemoveIgnored = CreateSettingsButton("Remove All", function()
		...
		wax.shared.Sonner.success("移除所有远程事件")
	end, Options.Section)
...
local ExecSupportSection = CreateSettingsSection("信息")
do
	Interface.New("TextLabel", {
		Text = table.concat({
			`执行器: <b>{wax.shared.ExecutorName}</b>`,
			`支持: {#wax.shared.ExecutorSupport.FailedChecks == 0 and "<b>支持全部</b>" or "<b>支持部分</b> (" .. #wax.shared.ExecutorSupport.FailedChecks .. " check(s) failed)"}`,
		}, "\n"),
		TextSize = 16,
		Size = UDim2.fromScale(1, 0),
		AutomaticSize = Enum.AutomaticSize.Y,
...
CreateSettingsCheckbox("PreferBufferFromString", {
	Text = "首选缓冲字符串(buffer.fromstring)",
	Default = false,
	Section = CodeGenSection,
})
...
local MainSection = CreateSettingsSection("主要")
...
CreateSettingsButton("清除日志", function()
	...
	wax.shared.Sonner.success("成功清理所有日志")
end, MainSection)

local FilterSection = CreateSettingsSection("过滤器")
...
CreateSettingsCheckbox("ShowExecutorLogs", {
	Text = `展示 {wax.shared.ExecutorName} 日志`,
	Default = true,
	Section = FilterSection,
	Callback = function()
...
CreateSettingsButton("重置为默认设置", function()
	...
	wax.shared.Sonner.success("成功重置!")
end, FilterSection)

local IgnoredSection = CreateSettingsSection("忽略")

CreateSettingsRemoteList("IgnoredRemotes", {
	Text = "忽略的事件",
	NullMessage = "目前没有忽略任何事件",
...
local BlockedSection = CreateSettingsSection("阻止")

CreateSettingsRemoteList("BlockedRemotes", {
	Text = "以阻止的事件",
	NullMessage = "还没有任何阻止的事件",
...
local LoggingSection = CreateSettingsSection("记录")
local SessionLog
...
CreateSettingsCheckbox("EnableLogging", {
	...
	Callback = function(value)
		if not value then
			...
			SessionLog.Text = `当前日志: <b>Not Logging</b>`
			wax.shared.Sonner.success("已禁用文件日志")
			return
		end

		local LogConnection = wax.shared.SetupLoggingConnection()
		SessionLog.Text = `当前日志: <b>{wax.shared.LogFileName:gsub("Cobalt/Logs/", "")}</b>`
		wax.shared.LogConnection = wax.shared.Connect(wax.shared.Communicator.Event:Connect(LogConnection))

		wax.shared.Sonner.success("已启用文件日志")
	end,
})

SessionLog = Interface.New("TextLabel", {
	Text = `当前日志: <b>{wax.shared.Settings.EnableLogging.Value and wax.shared.LogFileName:gsub(
		"Cobalt/Logs/",
		""
	) or "Not Logging"}</b>`,
	TextSize = 16,
...
local CopySessionName = CreateSettingsButton("复制日志名称", function()
	if not wax.shared.Settings.EnableLogging.Value then
		wax.shared.Sonner.error("文件日志记录没有启动")
		return
	end
	...
	if not Success then
		warn(Error)
		wax.shared.Sonner.error("无法复制名称")
		return
	end

	wax.shared.Sonner.success("成功复制名称到粘贴板")
end, SessionButtons, 14)
CopySessionName.Size = UDim2.new(0.5, -4, 0, 24)

local CopyFullSessionPath = CreateSettingsButton("复制路径", function()
	if not wax.shared.Settings.EnableLogging.Value then
		wax.shared.Sonner.error("文件日志记录没有启用")
		return
	end
	...
	if not Success then
		warn(Error)
		wax.shared.Sonner.error("无法复制路径")
		return
	end

	wax.shared.Sonner.success("成功复制路径到粘贴板")
end, SessionButtons, 14)
...
local Credits = {
	{
		Credit = '<font color="#3798ff">upio</font>',
		Description = 'Cobalt Developer',
	},
	{
		Credit = '<font color="#3798ff">deivid</font>', 
		Description = 'Cobalt Developer',
	},
	{
		Credit = "shadcn",
...
CreateInfoTab("ellipsis", "参数", ArgumentInfoUI)
...
CreateInfoTab("code", "代码", CodeInfoUI)
...
CreateInfoTab("parentheses", "函数信息", FunctionInfoUI)
...
CreateInfoDropdownButton("code", "代码", {
	{
		...
		Callback = function()
			...
			if Success then
				wax.shared.Sonner.success("复制代码到粘贴板")
			else
				wax.shared.Sonner.error("无法复制")
	...
	},
	{
		...
		Callback = function()
			...
			if Success then
				...
				wax.shared.Sonner.error("无法复制代码")
				warn("Failed to copy code to clipboard", Error)
			end
		end,
	},
	{
		...
		Callback = function()
			...
			if Success then
				wax.shared.Sonner.success("复制 Function info 到粘贴板")
			else
				wax.shared.Sonner.error("无法复制Function info")
...
CreateInfoDropdownButton("scroll-text", "来源", {
	{
		...
		Callback = function()
			...
			if Success then
				wax.shared.Sonner.success("已复制事件路径到粘贴板")
			else
				wax.shared.Sonner.error("无法复制事件路径到粘贴板")
	...
	},
	{
		...
		Callback = function()
			...
			if Success then
				wax.shared.Sonner.success("复制脚本路径到粘贴板")
			else
				wax.shared.Sonner.error("无法复制脚本路径到粘贴板")
    ...
    return CurrentInfo and typeof(CurrentInfo.Origin) == "Instance"
		...
		Callback = function()
    local scriptInst = CurrentInfo and CurrentInfo.Origin
    if not (scriptInst and typeof(scriptInst) == "Instance") then
        return
    end
    local ok, bytecode = pcall(getscriptbytecode, scriptInst)
    if not ok then
        wax.shared.Sonner.error("无法获取脚本字节码")
        return
    end
    ...
    if not requestFunc then
        wax.shared.Sonner.error("当前执行器不支持 HTTP 请求")
        return
    end
    local response = requestFunc({
        ...
        Body = game:GetService("HttpService"):JSONEncode({ script = base64_encode(bytecode) })
    })

    if response and response.StatusCode == 200 and response.Body then
        local copySuccess = pcall(setclipboard, response.Body)
        if copySuccess then
            wax.shared.Sonner.success("反编译代码已复制")
        end
    else
        local errMsg = response and response.Body
        wax.shared.Sonner.error("反编译失败: " .. tostring(errMsg))
    end
end,
	},
})
...
CreateInfoDropdownButton("network", "事件", {
	...
	},
	{
		Text = function()
			...
			return CurrentLog.Ignored and "取消忽略" or "忽略"
		end,
		Icon = function()
			if not CurrentLog then
				return "eye"
		...
		Callback = function()
			...
			wax.shared.Sonner.success(`{CurrentLog.Ignored and "开启" or "停止"} 忽略事件`)
		end,
	},
	{
		Text = function()
			...
			return CurrentLog.Blocked and "取消阻止" or "阻止"
		end,
		Icon = function()
			if not CurrentLog then
				return "lock"
		...
		Callback = function()
			...
			wax.shared.Sonner.success(`{CurrentLog.Blocked and "开启" or "关闭"} 阻止事件`)
		end,
	},
	{
		Text = "清除该事件日志",
		...
		Callback = function()
			...
			wax.shared.Sonner.success("清理日志事件成功")
		end,
	},
})
...
function CreatePaginationEllipsis(Order: number, Visible: boolean)
	...
	Ellipsis.Ellipsis.FocusLost:Connect(function(EnterPressed)
		...
		if not Page then
			wax.shared.Sonner.error("提供的页码无效!")
			Ellipsis.Text = ""
			return
		end
	...
	local LogContextMenu = CreateContextMenu(Button, {
		{
			...
			Callback = function()
				...
				wax.shared.Sonner.success(`{Log.Ignored and "开启" or "Stopped"} 忽略事件`)
			end,
		},
		{
			Text = function()
			...
			Callback = function()
				...
				wax.shared.Sonner.success(`{Log.Blocked and "开启" or "停止"} 阻止事件`)
			end,
		},
		{
			Text = "Clear Logs",
			...
			Callback = function()
				...
				wax.shared.Sonner.success("已清理日志中所有事件!")
			end,
		},
	}, true)
		...
		local CallFrameContextMenu = CreateContextMenu(CallFrame, {
			{
				...
				Callback = function()
					...
					if Success then
						...
						wax.shared.Sonner.error("无法复制代码到粘贴板")
						warn("Failed to copy code to clipboard", Error)
					end
				end,
			},
			...
			},
			{
				...
				Callback = function()
					...
					if Success then
						wax.shared.Sonner.success("复制事件路径到粘贴板")
					else
						wax.shared.Sonner.error("无法复制事件路径到粘贴板")
						warn("Failed to copy remote path to clipboard", Error)
					end
...
function OpenInfo(CallInfo)
	...
	for _, tabName in pairs({ "函数信息", "代码" }) do
		local tabData = InfoModalTab[tabName]
		if not tabData then
			continue
		end

		local tabButton = tabData.TabButton
		local tabContent = tabData.TabContents

		tabButton.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
		tabButton.Frame.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
		tabContent.Visible = false
	end

	CurrentInfoTab = "参数"
	if InfoModalTab["参数"] then
		local tabButton = InfoModalTab["参数"].TabButton
		local tabContent = InfoModalTab["参数"].TabContents
...
wax.shared.Connect(wax.shared.Communicator.Event:Connect(function(Instance, Type, CallIndex)
	if not (CurrentTab and TabNaTypTo[CurrentTab.Name] == Type) then
		return
	end
